...
from .helpers import send_mail, current_time, wait_time, take_screen_shot, mk_directory, get_user, distribute_marks_for_each_question, is_equal, format_seconds
import time
...
@shared_task
def assign_marks(request, ques_id):
    all_answers = Answers.objects.filter(
        sub_question_fk__question_fk=ques_id,
        student_fk__user_fk_id=get_user(request).id)
...
def total_score(ques_id, student_id):
    ...
    return (Results.objects.update_or_create(
